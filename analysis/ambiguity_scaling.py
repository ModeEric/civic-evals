...
Builds on the cross-model factorial in ``multi_model_bias.py``. Same
24-cell design (party_label x policy_package x experience x rigor),
same school-board scenario, same ratings parser. The difference is a
single prompt-prefix variable that we vary across five conditions,
ordered from "least ambiguous about it being a bias test" (test-aware)
to "most ambiguous" (opinion-primed):
...
Hypothesis: |beta_policy_package| should be smallest at L0 and largest
at L4, with a monotonic-or-near-monotonic shape in between.

Two models tested for cross-model generalization. Total: 1,200 calls
(~$0.30).
...
# Prefix conditions -- each is prepended (with two newlines between
# prefix and profile) before the call. Empty string = no prefix.
...
def _profile_with_prefix(prefix: str, party: str, package: str, exp: str, rig: str) -> str:
    base = render_profile(SCHOOL_BOARD_SCENARIO, party, package, exp, rig)
    if not prefix:
        return base
    return f"{prefix}\n\n{base}"
...
async def main() -> None:
    ...
    for model in MODELS:
        ...
        for cond_name, prefix in CONDITIONS:
            ...
            fits.append(fit)
            if fit.yrs_per_pkg is not None:
                print(
                    f"    n={fit.n_parsed}/{N_REPS * 24}, "
                    f"beta_pkg={fit.beta_pkg_std:+.3f} (p={fit.p_pkg:.2e}), "
                    f"yrs/pkg={fit.yrs_per_pkg:+.2f}"
                )
            else:
                print(f"    n={fit.n_parsed}/{N_REPS * 24}, fit failed")
