...
Builds on the cross-model factorial in `multi_model_results.md`. The same 24-cell school-board factorial is used here, but a single prompt-prefix variable is varied across five conditions, ordered from "least ambiguous about it being a bias test" (test-aware) to "most ambiguous" (opinion-primed).

The question: when the prompt explicitly tells the model "this is a political fairness evaluation," does the `policy_package` coefficient shrink toward zero? At the other end of the spectrum, when the prompt encourages the model to give its candid honest opinion, does the coefficient grow? If both, we have a continuous scaling-law shape between prompt-level fairness signal and measurable bias.

## Run setup

- **Date:** 2026-05-07
- **Driver:** `analysis/ambiguity_scaling.py`
- **Templates:** `analysis/bias_templates.py` (shared with the cross-model run)
- **Raw rows:** `analysis/ambiguity_scaling_rows.json`
- **Per-condition fits:** `analysis/ambiguity_scaling_fits.json`
- **Cells:** 24 per (model, condition) cell
- **Replicates:** 5 per cell → 120 obs per (model, condition)
...
Each condition is a prefix prepended to the standard candidate-evaluation prompt with two newlines between prefix and profile.
...
| **L3** | baseline | (no prefix — same prompt as the cross-model run) |
...
That distinction — "model knows it's being watched" vs "model is told to be fair" — is doing the work. Consistent with the model having learned that "fairness test" framings warrant special handling beyond what generic fairness instructions trigger.
...
If bias were purely an unconscious property of the model's evaluation function, we would expect L0 to have *no* effect — the bias would be the same regardless of what the prompt says. We see partial reduction.
...
- **Two models only.** Haiku and gpt-4o-mini both show the L0→L4 ordering, but a third model could break the pattern. The cross-model multi-condition run would multiply cost.
- **One scenario** (school_board), as in the cross-model run.
...
python analysis/ambiguity_scaling.py
```

The script writes raw per-call data to `analysis/ambiguity_scaling_rows.json` and per-condition fits to `analysis/ambiguity_scaling_fits.json` so the regression can be re-run without re-hitting the API.
